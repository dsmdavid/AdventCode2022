...
create or replace table TEST_DB.AOC2022.solved_day_8 as with sample_values as (
    ...
    all_visibility as (
        ...
        from sample_ as n
            left join rec on n.rn = rec.rn
            and n.cn = rec.cn --where rec.continue = 1
        group by 1,
            2,
            3
    )
...
/* split into 4 (up, down, left, right) seem to be more performant
 ...
 from source as i,
 lateral flatten(input=>split(regexp_replace(i.input, '.', ',\\0', 2), ',')) as f
 ),
 sample_ as (
 select * from narrow 
 --where rn= 2 and cn =4
 ...
 ups as (
 select sample_.rn, sample_.cn, sample_.height, 0 as ct, sample_.rn as n_rn, sample_.cn as n_cn, 1 as continue, 0 as n_height
 from sample_
 union all 
 select rec.rn, rec.cn, rec.height, rec.ct +1 as ct, n.rn as n_rn, n.cn as n_cn,
 case when rec.height <= n.height then 0 else 1 end, n.height as n_height
 from ups as rec
 inner join narrow as n 
 on rec.cn = n.cn
 and rec.n_rn - 1 = n.rn
 where 1=1 --ups.height >= n.height
 and rec.continue = 1
 ),
 downs as (
 select sample_.rn, sample_.cn, sample_.height, 0 as ct, sample_.rn as n_rn, sample_.cn as n_cn, 1 as continue, 0 as n_height
 ...
 n.height,
 max(ups.ct) as ups,
 max(downs.ct) as downs,
 max(lefts.ct) as lefts,
 max(rights.ct) as rights,
 ups * downs * lefts * rights as visibility
 from sample_ as n
 left join ups 
 on n.rn = ups.rn
 ...
 n.height,
 min(ups.n_rn) as ups,
 max(downs.n_rn) as downs,
 min(lefts.n_cn) as lefts,
 max(rights.n_cn) as rights,
 case 
 when ups = 1 then 1
 when downs = (select max(rn) from narrow) then 1
 when lefts = 1 then 1
 ...
 on n.rn = rights.rn
 and n.cn = rights.cn
 where ups.continue = 1
 and downs.continue = 1
 and lefts.continue = 1
 and rights.continue = 1
 group by 1,2,3
 )
 
 select 'part1' as part, count(*) as answer from seen_by
 where is_visible = 1
 union all
 select 
 'part2' as part, 
