...
class Monkey():
    def __init__ (self,name, starting_items, operation, divisible_by, monkey_if_true, monkey_if_false, decrease = 'old,3'):
        self.name = name
        self.list_of_items = deque(starting_items)
        self.worriness_increase_operator = MAP_OPERATOR[operation[0]]
        self.worriness_increase_value = None if operation[1] == 'old' else int(operation[1])
    ...
    def inspect_item(self, item):
        ...
        modifier = current_worriness if not self.worriness_increase_value else self.worriness_increase_value
...
#     If false: throw to monkey 4'''
    ...
    starting_items = list(map(int,block_lines[1].split(':')[1].replace(' ','').split(',')))
    ...
    monkey_if_false =  int(block_lines[5].replace('If false: throw to monkey','').strip())

    return (monkey, starting_items, operation, divisible_by, monkey_if_true, monkey_if_false)
