...
MAP_POINTS = {"N": (1, 0), "E": (0, 1), "S": (-1, 0), "W": (0, -1)}
MAP_DIRECTIONS = {
    "N": {"R": "E", "L": "W"},
    "E": {"R": "S", "L": "N"},
    "S": {"R": "W", "L": "E"},
...
MAP_BLIZZARD = {"<": "W", ">": "E", "^": "N", "v": "S"}
MAP_DIRECTION_SCORE = {"N": 3, "E": 0, "S": 1, "W": 2}
MAP_FACE = {
...
pattern = re.compile(
    r"([A-Z])\-([A-Z]).{3}([A-Z])\-([A-Z]).{2}([A-Z]).([A-Z]+).(\d+).{3}([A-Z]).([A-Z]+).(\d+)"
)
...
def create_grid_face():
    ...
    for item in GRID.keys():
        ...
        for k, v in MAP_FACE.items():
            if (v["ROW_MIN"] <= item[0] <= v["ROW_MAX"]) and (
                v["COL_MIN"] <= item[1] <= v["COL_MAX"]
            ):
                face = k
...
class Traveller:
    ...
    def wrap_next_available(self, type="flat"):
        ...
        if type == "flat":
            ...
            next_pos = sum_tup(
                (base_new_face_row, base_new_face_col), (new_offset_row, new_offset_col)
            )
            # - what is the new offset (and direction!)
            # direction, order,...
            ...
            if GRID[next_pos] == " ":
                ...
                print(
                    ...
                    new_face,
                    delta,
                    new_instructions["type"],
                    new_instructions["new_type"],
                    "\n",
                    "current_offsets:\t",
                    current_offset_row,
                    current_offset_col,
...
sample_instructions = (
    """10L5R100L1L1L15R1L3L19R20L100L17R5L1R10R1L15R4L100R3L100R1L12"""
)
